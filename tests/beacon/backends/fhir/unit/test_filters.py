...
def test_apply_filters_ontology_only_single_no_unsupported(single_disease_biosample_parameter):
    ...
    assert isinstance(result[0][0], DiagnosisBiosamples)
    assert result[0][0].conditions == single_disease_biosample_parameter['parameter'].conditions
    assert result[1] == []
...
def test_apply_filters_ontology_only_multiple_no_unsupported(multiple_diseases_biosample_parameter):
    ...
    result = apply_filters(filters)
    assert isinstance(result[0][0], DiagnosisBiosamples)
    assert isinstance(result[0][1], DiagnosisBiosamples)
    assert result[0][0].conditions == [multiple_diseases_biosample_parameter['parameter'].conditions[0]]
    assert result[0][1].conditions == [multiple_diseases_biosample_parameter['parameter'].conditions[1]]
    assert result[1] == []
...
def test_apply_filters_alphanumeric_only_single_no_unsupported(single_sex_biosample_parameter):
    ...
    result = apply_filters(filters)
    assert isinstance(result[0][0], SexBiosample)
    assert result[0][0].conditions == single_sex_biosample_parameter['parameter'].conditions
    assert result[1] == []
...
def test_apply_filters_alphanumeric_only_multiple_no_unsupported(multiple_sex_biosample_parameter):
    ...
    result = apply_filters(filters)
    assert isinstance(result[0][0], SexBiosample)
    assert result[0][0].conditions == multiple_sex_biosample_parameter['parameter'].conditions
    assert result[1] == []
...
def test_apply_filters_custom_only_single_no_unsupported(single_disease_biosample_parameter_as_custom):
    ...
    result = apply_filters(filters)
    assert isinstance(result[0][0], DiagnosisBiosamples)
    assert result[0][0].conditions == single_disease_biosample_parameter_as_custom[
...
def test_apply_filters_custom_only_multiple_no_unsupported(multiple_diseases_biosample_parameter_as_custom):
    ...
    assert isinstance(result[0][0], DiagnosisBiosamples)
    assert isinstance(result[0][1], DiagnosisBiosamples)
    assert result[0][0].conditions == [multiple_diseases_biosample_parameter_as_custom['parameter'].conditions[0]]
    assert result[0][1].conditions == [multiple_diseases_biosample_parameter_as_custom['parameter'].conditions[1]]
    assert result[1] == []
...
def test_apply_filters_miscellaneous_no_unsupported(multiple_diseases_biosample_parameter_miscellaneous_type):
    ...
    assert isinstance(result[0][0], DiagnosisBiosamples)
    assert isinstance(result[0][1], DiagnosisBiosamples)
    assert result[0][0].conditions == [
        multiple_diseases_biosample_parameter_miscellaneous_type['parameter'].conditions[0]]
    assert result[0][1].conditions == [
        multiple_diseases_biosample_parameter_miscellaneous_type['parameter'].conditions[1]]
    assert result[1] == []
...
def test_apply_ontology_filter(single_disease_biosample_parameter):
    ...
    parameters = dict()
    parameter = apply_ontology_filter(filter, list())
    assert parameter.conditions == [{'code': 'Orphanet_457260', 'code_system': 'ordo', 'extension': ''}]
...
def test_apply_ontology_filter_not_supported():
    ...
    parameter = apply_ontology_filter(filter, unsupported_filters)
...
def test_apply_alphanumeric_filter_string_value():
    ...
    parameter = apply_alphanumeric_filter(filter, list())
    assert parameter.conditions == [{'operator': Operator.EQUAL, 'value': 'female'}]
...
def test_apply_alphanumeric_filter_not_supported():
    ...
    unsupported_filters = list()
    parameter = apply_alphanumeric_filter(filter, unsupported_filters)
    assert parameter is None
    assert len(unsupported_filters) == 1
...
def test_apply_custom_filter_string_value():
    ...
    parameter = apply_custom_filter(filter, list())
    assert parameter.conditions == [{'code': 'Orphanet_166', 'code_system': 'ordo'}]
...
def test_apply_custom_filter_string_value_not_supported():
    ...
    unsupported_filters = list()
    parameter = apply_custom_filter(filter, unsupported_filters)
    assert parameter is None
    assert len(unsupported_filters) == 1
...
def test_apply_custom_filter_list_value_not_supported():
    ...
    unsupported_filters = list()
    parameter = apply_custom_filter(filter, unsupported_filters)
    assert parameter.conditions == [{'code': 'Orphanet_457260', 'code_system': 'ordo'}]
    assert len(unsupported_filters) == 1
