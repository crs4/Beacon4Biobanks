...
from tests.beacon.backends.fhir.integration.fixtures import query_single_orphacode_string_v3, \
    ...
    query_single_orphacode_string_v4, query_single_orphacode_array_v4, query_multiple_orphacodes_array, \
    query_multiple_orphacodes_array_v3_v4_combined, \
    query_sex_single_value, query_sex_multiple_value, query_sex_multiple_value_not_allowed, query_age_this_year, \
    query_unsupported_param, \
    query_both_supported_and_unsupported, query_disease_code_and_sex, query_sample_type_transcoded_to_single_code, \
    query_sample_type_transcoded_to_multiple_code, \
    query_empty_filter, query_age_of_diagnosis

uri = f'{get_base_uri()}/api/biosamples'
headers = get_headers()
...
def test_query_biosamples_by_single_orphacode_v3_string(query_single_orphacode_string_v3):
    r = requests.post(url=uri, headers=headers, json=get_request_body(query_single_orphacode_string_v3['query']),
                      verify=False)
    assert r.status_code == 200
    assert query_single_orphacode_string_v3['expected_biosamples_count'] == json.loads(r.content)['responseSummary'][
        "numTotalResults"]


def test_query_biosamples_by_single_orphacode_v3_array(query_single_orphacode_array_v3):
    r = requests.post(url=uri, headers=headers, json=get_request_body(query_single_orphacode_array_v3['query']),
                      verify=False)
    assert r.status_code == 200
    assert query_single_orphacode_array_v3['expected_biosamples_count'] == json.loads(r.content)['responseSummary'][
        "numTotalResults"]


def test_query_biosamples_by_single_orphacode_v4_string(query_single_orphacode_string_v4):
    r = requests.post(url=uri, headers=headers, json=get_request_body(query_single_orphacode_string_v4['query']),
                      verify=False)
    assert r.status_code == 200
    assert query_single_orphacode_string_v4['expected_biosamples_count'] == json.loads(r.content)['responseSummary'][
        "numTotalResults"]


def test_query_biosamples_by_single_orphacode_v4_array(query_single_orphacode_array_v4):
    r = requests.post(url=uri, headers=headers, json=get_request_body(query_single_orphacode_array_v4['query']),
                      verify=False)
    assert r.status_code == 200
    assert query_single_orphacode_array_v4['expected_biosamples_count'] == json.loads(r.content)['responseSummary'][
        "numTotalResults"]


def test_query_biosamples_multiple_orphacodes_array(query_multiple_orphacodes_array):
    r = requests.post(url=uri, headers=headers, json=get_request_body(query_multiple_orphacodes_array['query']),
                      verify=False)
    assert r.status_code == 200
    assert query_multiple_orphacodes_array['expected_biosamples_count'] == json.loads(r.content)['responseSummary'][
        "numTotalResults"]


def test_query_biosamples_multiple_orphacodes_array_v3_v4(query_multiple_orphacodes_array_v3_v4_combined):
...
def test_query_biosamples_sex_single_value(query_sex_single_value):
    r = requests.post(url=uri, headers=headers, json=get_request_body(query_sex_single_value['query']), verify=False)
    assert r.status_code == 200
    assert query_sex_single_value['expected_biosamples_count'] == json.loads(r.content)['responseSummary'][
        "numTotalResults"]


def test_query_biosamples_sex_multiple_value(query_sex_multiple_value):
    r = requests.post(url=uri, headers=headers, json=get_request_body(query_sex_multiple_value['query']), verify=False)
    assert r.status_code == 200
    assert query_sex_multiple_value['expected_biosamples_count'] == json.loads(r.content)['responseSummary'][
        "numTotalResults"]


def test_query_biosamples_sex_multiple_value_not_allowed(query_sex_multiple_value_not_allowed):
...
def test_query_biosamples_both_supported_and_unsupported(query_both_supported_and_unsupported):
    r = requests.post(url=uri, headers=headers, json=get_request_body(query_both_supported_and_unsupported['query']),
                      verify=False)
    assert r.status_code == 200
    assert query_both_supported_and_unsupported['expected_biosamples_count'] == \
...
def test_query_biosamples_by_disease_and_sex(query_disease_code_and_sex):
    r = requests.post(url=uri, headers=headers, json=get_request_body(query_disease_code_and_sex['query']),
                      verify=False)
    assert r.status_code == 200
    assert query_disease_code_and_sex['expected_biosamples_count'] == json.loads(r.content)['responseSummary'][
        "numTotalResults"]


def test_query_sample_type_single_code_transcoding(query_sample_type_transcoded_to_single_code):
    r = requests.post(url=uri, headers=headers,
                      json=get_request_body(query_sample_type_transcoded_to_single_code['query']), verify=False)
    assert r.status_code == 200
    assert query_sample_type_transcoded_to_single_code['expected_biosamples_count'] == \
           json.loads(r.content)['responseSummary']["numTotalResults"]


def test_query_sample_type_multiple_code_transcoding(query_sample_type_transcoded_to_multiple_code):
    r = requests.post(url=uri, headers=headers,
                      json=get_request_body(query_sample_type_transcoded_to_multiple_code['query']), verify=False)
    assert r.status_code == 200
    assert query_sample_type_transcoded_to_multiple_code['expected_biosamples_count'] == \
           json.loads(r.content)['responseSummary']["numTotalResults"]


def test_query_biosamples_empty_filter(query_empty_filter):
...
def test_query_biosamples_by_age_of_diagnosis(query_age_of_diagnosis):
    r = requests.post(url=uri, headers=headers, json=get_request_body(query_age_of_diagnosis['query']),
                      verify=False)
    assert r.status_code == 200
    assert query_age_of_diagnosis['expected_biosamples_count'] == json.loads(r.content)['responseSummary'][
        "numTotalResults"]
